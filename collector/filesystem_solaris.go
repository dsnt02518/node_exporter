...
const (
	defIgnoredMountPoints = "^/(dev|devices|proc|system)($|/)"
	defIgnoredFSTypes     = "^(autofs|ctfs|dev|devfs|fd|hsfs|lofs|mntfs|objfs|proc|sharefs|tmpfs)$"
)
...
func (c *filesystemCollector) GetStats() ([]filesystemStats, error) {
	...
	for _, labels := range mps {
		...
		buf := new(unix.Statvfs_t)
		err = unix.Statvfs(rootfsFilePath(labels.mountPoint), buf)
		stuckMountsMtx.Lock()
		close(success)
		...
		if err != nil {
			...
			level.Debug(c.logger).Log("msg", "Error on statvfs() system call", "rootfs", rootfsFilePath(labels.mountPoint), "err", err)
			continue
		}
...
func parseFilesystemLabels(r io.Reader) ([]filesystemLabels, error) {
	...
	for scanner.Scan() {
		...
		// We throw away parts[4], which is the mnt_time field of the mntent structure.
